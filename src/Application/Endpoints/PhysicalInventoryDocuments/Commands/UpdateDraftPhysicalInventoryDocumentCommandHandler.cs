...
{
    public class UpdateDraftPhysicalInventoryDocumentCommandHandler : IRequestHandler<UpdateDraftPhysicalInventoryDocumentCommand, EndpointResult<PhysicalInventoryDocumentViewModel>>
    {
        private readonly IRequestValidator<UpdateDraftPhysicalInventoryDocumentCommand> _requestValidator;
        private readonly IRepositoryWrapper _repository;
        private readonly IMapper _mapper;
        private readonly IEntityMapperService _entityMapperService;

        public UpdateDraftPhysicalInventoryDocumentCommandHandler(IRequestValidator<UpdateDraftPhysicalInventoryDocumentCommand> requestValidator, IRepositoryWrapper repository, IMapper mapper, IEntityMapperService entityMapperService)
        {
        ...
        public async Task<EndpointResult<PhysicalInventoryDocumentViewModel>> Handle(UpdateDraftPhysicalInventoryDocumentCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var sourcePID = await _repository.PhysicalInventoryDocument.GetAsync(q => q.Id == pidToUpdate.Id && q.RowStatus == 0 && q.IsDraft == true, cancellationToken);
                if (sourcePID == null)
