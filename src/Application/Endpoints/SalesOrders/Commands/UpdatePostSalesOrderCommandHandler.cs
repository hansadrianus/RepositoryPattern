...
{
    public class UpdatePostSalesOrderCommandHandler : IRequestHandler<UpdatePostSalesOrderCommand, EndpointResult<SalesOrderViewModel>>
    {
        private readonly IRequestValidator<UpdatePostSalesOrderCommand> _requestValidator;
        private readonly IRepositoryWrapper _repository;
        private readonly IMapper _mapper;
        private readonly IEntityMapperService _entityMapperService;

        public UpdatePostSalesOrderCommandHandler(IRequestValidator<UpdatePostSalesOrderCommand> requestValidator, IRepositoryWrapper repository, IMapper mapper, IEntityMapperService entityMapperService)
        {
        ...
        public async Task<EndpointResult<SalesOrderViewModel>> Handle(UpdatePostSalesOrderCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var sourceOrder = await _repository.SalesOrder.GetAsync(q => q.Id == orderToUpdate.Id && q.RowStatus == 0 && q.IsDraft == false, cancellationToken);
                if (sourceOrder == null)
